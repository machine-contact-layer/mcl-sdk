...
## Physical contention instrumentation

`--capture <file.wav>` retains the actual microphone samples, including local
transmission, with a hard 300-second recording limit. The runtime still excludes
self-transmission from protocol decoding. These are private room recordings;
they are not synthetic modem fixtures. `--first-slot 0` supplies only the first
one-byte contention draw for a disclosed collision test; later randomness and
the source reference remain independently generated by the OS. It must be named
in the test receipt and is not a production configuration recommendation.

Windows can qualify as a third physical AP contention participant without
being a complete BLE port. Qualification requires actual room-audio evidence;
the codec check alone is insufficient. A selected DFR/Android pair must still
validate and migrate. See the current core governance section 5.10.
