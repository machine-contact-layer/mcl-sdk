<#
    Write the MCL autonomous node application to the DFR1154 application partition.
...
if ([string]::IsNullOrWhiteSpace($Image)) {
    $Image = Join-Path $scriptDir 'build\out\mcl-auto-node-staging.ino.bin'
}
...
# app3M_fat9M_16MB gives the application 0x300000 bytes.
$appPartitionSize = 0x300000
...
if ($flashExit -ne 0) { throw "esptool write-flash failed with exit code $flashExit" }
